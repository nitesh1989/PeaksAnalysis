...
###############################################################################
my.path ="~/TestRun/COPD-LungCancer-PeaksAnalysis"
setwd(my.path)
###############################################################################

# Set Path of Files to Make unique and Select Sample set
###############################################################################
files.load.path = "data/raw"
files.save.path = "data/processed_unique"
sample.set = "COPD" # Make Function argument
###############################################################################

# List Peaks Mappings file names
###############################################################################
mappings = list.files(file.path(my.path,files.load.path,sample.set),pattern = ".txt",full.names=TRUE)
###############################################################################
...
for i in (1:length(mappings)) { 
    ...
    dat.reduce = dat[! duplicated(dat$ncbi_gene_id),]
    dat.reduce = dat.reduce[! is.na(dat.reduce$ncbi_gene_id),]
    str(dat.reduce)
}
